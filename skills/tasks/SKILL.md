...
Both files are append-only (closing a todo MOVES it; it never disappears). the owner can `cat` them from any terminal; you can read them via `bash_exec` from your workspace pwd.
...
- **example-project VET** flagged an issue as "proceed" and now needs code work → `source: 'project'`
- **Chat** — the user said "later" or you promised to circle back → `source: 'chat'`
- **Anywhere else** — `source: 'manual'`

Text must be ONE LINE, ≤ 280 chars, action-verb first. Bad: "the report". Good: "Send weekly report to Alex (CC the owner)".

## When to call `todo_list`

**Before adding** — to avoid duplicates. **At the start of an idle/scheduled turn** — to see whether you already owe the owner something. **When asked "what's open"** — return the list.
...
```text
- [01KRJK7A8M00000000000000] 2026-05-15T07:31:22Z [email] Reply to Alex re Q2 invoice (await the owner approval)
- [01KRJK7A8M00000000000001] 2026-05-15T08:00:01Z [project] Open draft PR for issue #142 (validate Apple Intelligence gating)
